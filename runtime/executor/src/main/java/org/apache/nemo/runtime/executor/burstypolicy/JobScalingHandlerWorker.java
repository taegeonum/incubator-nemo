...
public final class JobScalingHandlerWorker implements TaskOffloadingPolicy {
  ...
  private synchronized void scaleOutToVms(final Map<String, List<String>> offloadTasks,
    ...
    final int numWorkers = totalOffloadTasks / evalConf.taskSlot;
    LOG.info("# of vm workers: {}, slot: {}, tasks: {}", numWorkers, evalConf.taskSlot, totalOffloadTasks);
        ...
        case RequestScaling:
          ...
          if (scalingMsg.getIsScaleOut()) {
            ...
            if (scalingMsg.hasDivideNum()) {
              ...
              final Map<String, String> taskExecutorIdMap = new HashMap<>();
              ...
              for (final ControlMessage.TaskLocation taskLocation : taskLocations) {
                ...
                taskExecutorIdMap.put(taskLocation.getTaskId(),taskLocation.getExecutorId());
              }
