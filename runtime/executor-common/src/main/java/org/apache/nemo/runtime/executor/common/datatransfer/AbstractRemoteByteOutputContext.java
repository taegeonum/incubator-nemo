...
import static org.apache.nemo.runtime.executor.common.ChannelStatus.WAIT_FOR_INPUT_RESTART;
...
public abstract class AbstractRemoteByteOutputContext extends AbstractByteTransferContext implements ByteOutputContext {
  ...
  public synchronized void receiveStopSignalFromChild(final ByteTransferContextSetupMessage msg, final TaskLoc sdt) {
    //TODO: location 바꿔줘야함.
    taskLocationMap.locationMap.put(msg.getTaskId(), sdt);
  ...
  public synchronized void receiveRestartSignalFromChild(Channel c,
    ...
    switch (channelStatus) {
      ...
      case OUTPUT_STOP: {
        throw new RuntimeException("Canot happen output stop when receiving input restart signal");
      }
      default:
        throw new RuntimeException("ChannelStatus should not be running " + channelStatus);
    }
  }
  ...
  public synchronized void restart(final String taskId) {
    ...
    if (channelStatus != ChannelStatus.OUTPUT_STOP) {
     throw new RuntimeException("Channel statis exception " + channelStatus);
    }
